...
void OnPaint(HDC& hdc)
{
	...
	origin.set_x(rect.left + (rect.right - rect.left) / 2.0f);
	origin.set_y(rect.top + (rect.bottom - rect.top) / 2.0f);
	g_screenCoordinate.SetInfo(Vector2D(50, 0), Vector2D(0, -50), origin);

	Basis2D basis2;
	basis2.SetInfo(Vector2D(2, 1), Vector2D(-1, 1));
	SetBasis2(basis2);

	DrawGrid(hdc, 10, 10);
	DrawAxis(hdc, 10, 10);
	DrawLine(hdc, Vector2D(0, 0), Vector2D(3, -2), 2, PS_SOLID, RGB(255, 0, 0));
}
