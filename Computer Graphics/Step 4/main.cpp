...
void OnIdle(float fElapsedTime)
{
	...
	{
		// For Animation
		// 기존 벡터가 기저에 의해 얼마나 바뀌었는지 체크하기 위해 애니메이션 처리

		const float animSpeed = 0.5f;
		static float lerpTime = 0.0f;  // 보간 시간 -> 시간이 선형보간 함수의 ratio기준이다.
		lerpTime += fElapsedTime * animSpeed; 
		Basis2D     basis2;

		Vector2D    axis0Begin = Vector2D(1, 0);
		Vector2D    axis0End = Vector2D(2, 1);
		Vector2D    axis1Begin = Vector2D(0, 1);
		Vector2D    axis1End = Vector2D(-1, 1);
	...
	// 기저에 의해 위치가 바뀐 축과 벡터 그리기
	{
		Basis2D     basis2;
		basis2.SetInfo(Vector2D(1, 0), Vector2D(0, 1));
		SetBasis2(basis2);

		Matrix2D    transform = Matrix2D(2, -1, 1, 1);

		DrawGrid(g_hdc, 20, 10);
		DrawAxis(g_hdc, 20, 10, RGB(255, 0, 0), RGB(255, 0, 0));
		DrawLine(g_hdc, transform* Vector2D(0, 0), transform*Vector2D(3, -2), 1, PS_DASH);
	}
